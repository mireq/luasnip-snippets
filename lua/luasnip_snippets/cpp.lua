...
ls.add_snippets("cpp", {
	s({trig = "ns", descr = "(ns) \"namespace .. (namespace)\"", priority = 0, trigEngine = te("!")}, {
		t"namespace", tr(1, ".+", " "), d(1, function(args, snip) return sn(nil, { i(1, jt({c_py({"cpp", 1}, "snip.rv = snip.basename or \"name\"", python_globals, args, snip, "", am[1])}, ""), {key = "i1"}) }) end), nl(),
		t"{", nl(),
		t"\t", i(0, "", {key = "i0"}), nl(),
		t"}", tr(1, ".+", " \\/* "), cp(1), tr(1, ".+", " *\\/")
	}),
	...
	s({trig = "itera", descr = "(itera) \"Auto iterator\"", priority = 0, trigEngine = te("")}, {
		...
		t"\t", t"std::cout << *", cp(1), t" << std::endl;", nl(),
		t"}"
	}),
	...
	s({trig = "forc", descr = "(forc) \"general for loop (for)\"", priority = -49, trigEngine = te("")}, {
		t"for (", i(6, "auto", {key = "i6"}), t" ", i(1, "i", {key = "i1"}), t" = ", i(2, "v.begin()", {key = "i2"}), t"; ", f(function(args, snip) return c_py({"cpp", 15}, "import re; snip.rv = re.split(\"[^\\w]\",t[1])[-1]", python_globals, args, snip, "", am[15]) end, ae(am[15])), t" ", i(4, "!=", {key = "i4"}), t" ", d(3, function(args, snip) return sn(nil, { i(1, jt({c_py({"cpp", 15}, "m = re.search(r\'^(?:(.*)(\\.|->)begin\\(\\)|((?:std|boost)::)?begin\\((.*)\\))$\', t[2]); snip.rv = (((m.group(3) if m.group(3) else \"\") + \"end(\" + m.group(4) + \")\") if m.group(4) else (m.group(1) + m.group(2) + \"end()\")) if m else \"\"", python_globals, args, snip, "", am[15])}, ""), {key = "i3"}) }) end), t"; ", d(5, function(args, snip) return sn(nil, { i(1, jt({"++", c_py({"cpp", 15}, "snip.rv = t[1].split(\" \")[-1]", python_globals, args, snip, "", am[15])}, ""), {key = "i5"}) }) end), t") {", nl(),
	...
	s({trig = "cl", descr = "(cl) \"class .. (class)\"", priority = -49, trigEngine = te("")}, {
		t"class ", d(1, function(args, snip) return sn(nil, { i(1, jt({c_py({"cpp", 17}, "snip.rv = snip.basename or \"name\"", python_globals, args, snip, "", am[17])}, ""), {key = "i1"}) }) end), nl(),
		t"{", nl(),
		t"public:", nl(),
	...
	s({trig = "using", descr = "(using) \"using directive/using declaration/type alias\"", priority = -49, trigEngine = te("")}, {
		t"using ", i(1, "namespace", {key = "i1"}), f(function(args, snip) return c_py({"cpp", 19}, "snip.rv = \' \' if t[1] == \'namespace\' else \' = \' if t[1] != \'\' else \'\'", python_globals, args, snip, "", am[19]) end, ae(am[19])), i(2, "name", {key = "i2"}), t";"
	}),
	...
	s({trig = "fnc", descr = "(fnc) \"Basic c++ doxygen function template\"", priority = -49, trigEngine = te("b")}, {
		...
		t"*", nl(),
		t"* @param: ", f(function(args, snip) return c_py({"cpp", 25}, "write_docstring_args(t[3],snip)", python_globals, args, snip, "", am[25]) end, ae(am[25])), nl(),
		t"*", nl(),
		t"* @return: ", f(function(args, snip) return c_py({"cpp", 25}, "snip.rv = t[1]", python_globals, args, snip, "", am[25]) end, ae(am[25])), nl(),
		t"*/", nl(),
