...
ls.add_snippets("xslt", {
	...
	s({trig = "template", descr = "(template) named", priority = -1000, trigEngine = te("w")}, c(1, {
		{
			t"<xsl:template match=\"", i(1, "*", {key = "i1"}), t"\">", i(2, "", {key = "i2"}), nl(),
			t"</xsl:template>", nl()
		},
		{
			t"<xsl:template name=\"", i(1, "name", {key = "i1"}), t"\">", i(2, "", {key = "i2"}), nl(),
			t"</xsl:template>", nl()
		},
	})),
	...
	s({trig = "variable", descr = "(variable) select", priority = -1000, trigEngine = te("w")}, c(1, {
		{
			t"<xsl:variable name=\"", i(1, "name", {key = "i1"}), t"\">", i(2, "", {key = "i2"}), nl(),
			t"</xsl:variable>", nl()
		},
		{
	...
	s({trig = "with-param", descr = "(with-param) select", priority = -1000, trigEngine = te("w")}, c(1, {
		{
			t"<xsl:with-param name=\"", i(1, "name", {key = "i1"}), t"\">", i(2, "", {key = "i2"}), t"</xsl:with-param>", nl()
		},
		{
			t"<xsl:with-param name=\"", i(1, "name", {key = "i1"}), t"\" select=\"", i(2, "*", {key = "i2"}), t"\" />", nl()
		},
	})),
})
