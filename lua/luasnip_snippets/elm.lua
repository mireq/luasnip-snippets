...
ls.add_snippets("elm", {
	s({trig = "impa", descr = "(impa) \"Qualified import\"", priority = -50, trigEngine = te("")}, {
		t"import ", i(1, "Json.Encode", {key = "i1"}), t" as ", d(2, function(args, snip) return sn(nil, { i(1, jt({c_py({"elm", 1}, "snip.rv = t[1].split(\".\")[-1]", python_globals, args, snip, "", am[1])}, ""), {key = "i2"}) }) end)
	}),
	s({trig = "impae", descr = "(impae) \"Qualified import with exposing\"", priority = -50, trigEngine = te("")}, {
		t"import ", i(1, "Json.Encode", {key = "i1"}), t" as ", d(2, function(args, snip) return sn(nil, { i(1, jt({c_py({"elm", 2}, "snip.rv = t[1].split(\".\")[-1]", python_globals, args, snip, "", am[2])}, ""), {key = "i2"}) }) end), t" exposing (", i(0, "Value", {key = "i0"}), t")"
	}),
